...
COPY env-var-check-task.sh /usr/src/app/env-var-check-task.sh
...
COPY --from=build /usr/src/app/init.sh /usr/local/lib/init.sh
COPY --from=build /usr/src/app/env-var-check-task.sh /usr/local/lib/env-var-check-task.sh
RUN chmod 0755 /usr/local/lib/init.sh
RUN chmod 0755 /usr/local/lib/env-var-check-task.sh

COPY --from=build /usr/src/app/cron-job /etc/cron.d/cron-job
